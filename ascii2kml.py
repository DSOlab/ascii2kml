...
class Placemark:
    ...
    def flush(self):
        ...
        print(
            "    <Point><coordinates>{:},{:},0.0</coordinates></Point>".format(
...
def parse_line(line, names_w_spaces=False, description_col=None):
    ...
    try:
        ...
        ermsg = (
            "[ERROR] Failed to resolve lat/lon fields in line \"{:}\"".format(
                line.strip()))
        # printer(ermsg)
        raise RuntimeError(ermsg)
    ##  resolve description
    if description_col and len(l) > description_col + idx:
        ...
        description = line[line.find(" {:} ".format(l[description_col])) +
                           1:].rstrip()
    else:
        description = ""
    ##  return Placemark
...
def flush_placemark(lat, lon, name="", description=""):
    ...
    print("    <Point><coordinates>{:},{:},0.0</coordinates></Point>".format(
        lon, lat))
    print("  </Placemark>")
    return
...
parser.add_argument(
    ...
    action='store_true',
    help=
    ('If set then the station/point names can include whitespace/tab '
...
parser.add_argument(
    ...
    required=False,
    help=(
        'If given, then the text starting at column DESCRIPTION_COL and '
...
parser.add_argument(
    ...
    action='store_true',
    help=(
        'By default the program will stop if it encounters a line it cannot '
...
for line in sys.stdin:
    if not line.lstrip().startswith('#'):
        try:
            placemark = parse_line(line, args.names_w_spaces,
