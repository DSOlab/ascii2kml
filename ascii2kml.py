...
class Placemark:
    ...
    def flush(self):
        ...
        print("    <Point><coordinates>{:},{:},0.0</coordinates></Point>".format(self.longtitude, self.latitude))
        print("  </Placemark>")
        return
...
def parse_line(line, names_w_spaces=False, description_col=None):
    l = line.split()
    if len(l) < 3:
        ermsg = "[ERROR] Number of fields in line \"{:}\" < 3".format(line)
        printer(ermsg)
    ...
    if names_w_spaces:
        ...
        for idx, colstr in enumerate(l):
            if is_float_with_dot(colstr):
                ...
                break
        if idx > len(l) - 2:
            ermsg = "[ERROR] Failed to resolve line \"{:}\". Cannot find lat/lon after name".format(line)
            printer(ermsg)
            raise RuntimeError(ermsg)
    ...
    try:
      lat, lon = [ float(x) for x in [l[idx], l[idx+1]] ]
          ...
          ermsg = "[ERROR] Failed to resolve lat/lon fields in line \"{:}\"".format(line)
          printer(ermsg)
          raise RuntimeError(ermsg)
    ##  resolve description
    if description_col and len(l) > description_col+idx:
      description_col += idx-1
      description = line[line.find(" {:} ".format(l[description_col]))+1:].rstrip()
    else:
      description = ""
    ##  return Placemark
...
parser = argparse.ArgumentParser(
    ...
    description='Read in points (on the earth\'s surface) and transform them to Placemarks in a valid KML document.',
...
parser.add_argument('--name-w-spaces',
    #metavar='NAMES_INCLUDE_WHITESPACES',
    dest='names_w_spaces',
    action='store_true',
    help='If set then the station/point names can include whitespaces. That means, that the name of each point will start from the begining of the line and stop untill we reach a column that can be interpreted as float (i.e. the latitude).')
